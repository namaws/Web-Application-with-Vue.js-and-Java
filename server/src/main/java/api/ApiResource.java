...
public class ApiResource {
    ...
    public Category categoryByName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            if (result == null) {
                throw new ApiException(String.format("No such categoryName: %s", categoryName));
            }
            return result;
        } catch (Exception e) {
            throw new ApiException(String.format("Category lookup by categoryName %s failed", categoryName), e);
        }
    }
